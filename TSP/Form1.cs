...
namespace TSP
{
    public partial class Form1 : Form
    {
        ...
        private void InitializeUI()
        {
            ...
            cmbSelection.Items.AddRange(new string[] { "roulette", "tournament", "rank" });
            ...
            cmbCrossover.Items.AddRange(new string[] { "ox1", "pmx" });
            ...
            cmbMutation.Items.AddRange(new string[] { "swap", "scramble" });
            ...
            lblIteration = new Label() { Text = "Iteracja: 0", Location = new Point(380, 730), AutoSize = true };
        ...
        {
            string sciezka = "C:\\Users\\filip\\Desktop\\studia\\semestr_6\\InteligencjaObliczeniowa\\TSP\\TSP\\wyniki.csv";
            bool plikIstnieje = File.Exists(sciezka);

            using (StreamWriter sw = new StreamWriter(sciezka, true))
            {
                if (!plikIstnieje)
                {
                    sw.WriteLine("populationID,czas_dzialania_s,iteracje,limit_stagnacji,ilosc_iteracji,fitness_koncowy,rozm_populacji,liczba_genow,selekcja,krzyzowanie,mutacja,procent_rodzicow,procent_mutacji,procent_krzyzowania");
                }

                string populationID = population.populationID;
                double czasDzialania = (DateTime.Now - startTime).TotalSeconds;

                int iteracje = generations;
                int limit = stagnationLimit;
                int fitnessKoncowy = bestFitness;
                int rozmPopulacji = population.basePopulation.Count;
                int liczbaGenow = population.basePopulation[0].genes.Count;

                string selekcja = selectionType;
                string krzyzowanie = crossoverType;
                string mutacja = mutationType;

                double procentRodzicow = PartentRatio;
                double procentMutacji = MutationRatio;
                double procentKrzyzowania = CrossoverRatio;


                sw.WriteLine($"{populationID},{czasDzialania:F2},{iteracje},{limit},{_lastGeneration},{fitnessKoncowy},{rozmPopulacji},{liczbaGenow},{selekcja},{krzyzowanie},{mutacja},{procentRodzicow},{procentMutacji},{procentKrzyzowania}");
            }
        }
